      ...
      <div>
        <div class="paging">
          <g:if test="${haveResults}">
           ...
           <table class="tt" cellspacing="0" align="center">
     <thead>
          <tr>
              <g:sortableColumn property="title" title="Link" style="width: 150px" />
              <g:sortableColumn property="title" title="Name" style="width: 100px" />
          </tr>
      </thead>
      <tbody>
      <div class="results">
        ...
        <tr>
          <div class="result">
            <g:set var="className" value="${ClassUtils.getShortName(result.getClass())}" />
            <g:set var="link" value="${createLink(controller: className[0].toLowerCase() + className[1..-1], action: 'show', id: result.id)}" />
            <pre><td><div class="name"><a href="${link}">${className} #${result.id}        </a></div></td></pre>
            <g:set var="desc" value="${result.toString()}" />
            <g:if test="${desc.size() > 120}"><g:set var="desc" value="${desc[0..120] + '...'}" /></g:if>
